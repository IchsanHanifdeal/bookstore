                        ...
                        @foreach ($cartItems as $cartItem)
                            <tr id="row-{{ $cartItem->id }}" class="border-b hover:bg-base-200">
                                <td class="p-5 flex items-center gap-5">
                                    <img src="{{ str_contains($cartItem->bukus->gambar, 'https://') ? $cartItem->bukus->gambar : asset('storage/' . $cartItem->bukus->gambar) }}"
                                        alt="{{ $cartItem->bukus->judul }}" class="w-20 h-20 rounded-xl shadow-lg" />
                                    <div>
                                        <span
                                            class="block font-semibold text-xl">{{ $cartItem->bukus->judul_buku }}</span>
                                        <span
                                            class="block text-sm text-gray-500">{{ $cartItem->bukus->penerbits->nama }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="p-5 text-center text-lg font-medium">
                                    Rp<span
                                        id="price-{{ $cartItem->id }}">{{ number_format($cartItem->bukus->harga, 0, ',', '.') }}</span>
                                </td>
    ...
    <script>
        function openPembayaranModal(cartItems) {
            ...
            Object.values(cartItems).forEach(item => {
                if (!item || !item.bukus) return; // Pastikan item memiliki data buku

                const judulBuku = item.bukus.judul_buku || 'Tidak diketahui';
                const harga = item.bukus.harga || 0;
