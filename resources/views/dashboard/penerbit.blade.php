<x-dashboard.main title="Penerbit">
    <div class="grid sm:grid-cols-2 gap-5 md:gap-6">
        @foreach (['total_penerbit_terdaftar', 'penerbit_terbaru'] as $type)
            <div class="flex items-center px-4 py-3 bg-white border-back rounded-xl">
                <span
                    class="
                  {{ $type == 'total_penerbit_terdaftar' ? 'bg-blue-300' : '' }}
                  {{ $type == 'penerbit_terbaru' ? 'bg-amber-300' : '' }}
                ...
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-700 line-clamp-1">
                        {{ $type == 'total_penerbit_terdaftar' ? $penerbit->count() ?? '-' : '' }}
                        {{ $type == 'penerbit_terbaru' ? $penerbit_terbaru->nama ?? 'Belum ada Penerbit Terbaru' : '' }}
                    </p>
                </div>
            </div>
        @endforeach
    ...
    <div class="flex flex-col lg:flex-row gap-5">
        @foreach (['tambah_penerbit'] as $item)
            <div onclick="{{ $item . '_modal' }}.showModal()"
                ...
                <div>
                    ...
                    <p class="text-sm opacity-60">
                        {{ $item == 'tambah_penerbit' ? 'Menambahkan penerbit untuk buku' : '' }}
                    </p>
                </div>
                <x-lucide-plus class="{{ $item == 'tambah_penerbit' ? '' : 'hidden' }} size-5 sm:size-7 opacity-60" />
            </div>
        @endforeach
    </div>

    <div class="flex gap-5">
        @foreach (['manajemen_penerbit_buku'] as $item)
            <div class="flex flex-col border-back rounded-xl w-full">
                <div class="p-5 sm:p-7 bg-white rounded-t-xl">
                    ...
                    <p class="text-sm opacity-60">
                        Kelola lebih mudah dengan memberi penerbit pada Buku.
                    </p>
                </div>
                <div class="w-full px-5 sm:px-7 bg-zinc-50">
                    <input type="text" placeholder="Cari data disini...." name="penerbit" id="searchInput"
                        ...
                        <table class="table table-zebra" id="dataTable">
                            <thead>
                                <tr class="text-center">
                                    @foreach (['no', 'Nama Penerbit', 'Alamat', 'Email', 'No Telp', 'register at', 'updated at', ''] as $item)
                                        <th class="uppercase font-bold">{{ $item }}</th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($penerbit as $i => $item)
                                    <tr>
                                        ...
                                        <td class="flex items-center gap-4">
                                            ...
                                            <dialog id="update_modal_{{ $item->id }}"
                                                ...
                                                <form method="POST" class="modal-box rounded-xl shadow-lg"
                                                    action="{{ route('update.penerbit', $item->id) }}"
                                                    enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <h3 class="text-xl font-bold text-gray-800">Update Penerbit</h3>
                                            ...
                                            <x-lucide-trash class="size-5 hover:stroke-red-500 cursor-pointer"
                                                onclick="initDelete('penerbit', {{ $item }})" />
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
<dialog id="tambah_penerbit_modal" class="modal modal-bottom sm:modal-middle">
    <form method="POST" class="modal-box rounded-xl shadow-lg" action="{{ route('store.penerbit') }}"
        enctype="multipart/form-data">
        @csrf
        <h3 class="text-xl font-bold text-gray-800">Tambah Penerbit</h3>
        <div class="modal-body mt-5 space-y-4">
            @csrf
            @foreach (['nama', 'alamat', 'email', 'telp'] as $type)
                <div class="flex flex-col gap-2">
        ...
        <div class="modal-action mt-6">
            <button type="button" onclick="document.getElementById('tambah_penerbit_modal').close()"
                class="btn bg-gray-200 text-gray-700 hover:bg-gray-300 border-0">
                Batal
            </button>
