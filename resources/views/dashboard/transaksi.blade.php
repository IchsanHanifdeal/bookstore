                            ...
                            <tbody>
                                @foreach ($transaksi as $i => $item)
                                    <tr>
                                        ...
                                        <td class="font-semibold uppercase text-center">{{ $item->bukus->judul_buku }}
                                        </td>
        ...
        <dialog id="{{ $action }}_modal_{{ $item->id }}" class="modal modal-bottom sm:modal-middle">
            <form action="{{ route($action . '.' . 'transaksi', ['id' => $item->id]) }}" method="POST"
                ...
                <div class="modal-body mb-6 text-gray-600">
                    <div class="mb-4">
                        <p class="text-base">
                            ...
                            <strong>{{ $item->bukus->judul_buku }}</strong> pada tanggal
                ...
                <div class="modal-action flex justify-end gap-4">
                    ...
                    <button type="submit"
                        class="btn btn-primary capitalize bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-md transition-all duration-300" onclick="closeAllModals(event)">
                        {{ ucfirst($action) }}
                    </button>
                </div>
            </form>
